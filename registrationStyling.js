...
const retieveEntries = () => {
  let entries = localStorage.getItem("userEntrie");
  if (entries) 
  {
...
const displayEntries = () => {
  ...
  let details = document.getElementById("user-entrie");
...
const saveUserForm = (event) => {
  ...
  if (age < 18 || age > 55) {
  ...
  else
   {
    ...
    userEntrie = retieveEntries();
    userEntrie.push(entry);
    localStorage.setItem("userEntrie", JSON.stringify(userEntrie));
    displayEntries();
    userForm.reset();
  }
};
